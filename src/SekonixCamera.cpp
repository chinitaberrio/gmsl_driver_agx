...
SekonixCamera::SekonixCamera(ros::NodeHandle &nh_in, DeviceArguments CameraArguments) : nh_(nh_in) {
  ...
  nh_in.param("/sekonix_camera/image_width", image_width_, int(640));
  nh_in.param("/sekonix_camera/image_height", image_height_, int(480));
  nh_in.param("/sekonix_camera/image_buffer", pub_buffer_, int(5));
  nh_in.param("/sekonix_camera/image_compressed", img_compressed_, true);
  nh_in.param("/sekonix_camera/image_compressed_quality", jpeg_quality_, int(70));
  ...
  nh_in.param<std::string>("/sekonix_camera/type_a", type_a_value,"ar0231-rccb-bae-sf3324");
  nh_in.param<std::string>("/sekonix_camera/type_b", type_b_value,"ar0231-rccb-bae-sf3324");
  nh_in.param<std::string>("/sekonix_camera/type_c", type_c_value,"ar0231-rccb-bae-sf3324");
  nh_in.param<std::string>("/sekonix_camera/type_d", type_d_value,"ar0231-rccb-bae-sf3324");
  nh_in.param<std::string>("/sekonix_camera/selector_mask", selector_mask_value,"0001");
  nh_in.param<std::string>("/sekonix_camera/cross_csi_sync", cross_csi_sync_value,"0");
  nh_in.param<std::string>("/sekonix_camera/fifo_size", fifo_size_value,"3");
  nh_in.param<std::string>("/sekonix_camera/slave", slave_value,"0");
