...
typedef enum {
     ...
     */
    NVM_ISP_NVSCISYNC_PROCESS_EOF_FENCE,
    /**
 ...
 *
 * \param[in] isp An NvMedia ISP %NvMediaISP handle.
 * \param[out] attrlist A pointer to an \ref NvSciSyncAttrList structure where
 ...
 *         invalid.
 * - ::NVMEDIA_STATUS_ERROR if there is any error.
 */
NvMediaStatus
...
/**
 * \brief Registers an \ref NvSciSyncObj with NvMediaISP.
 *
 ...
 *
 * \param[in] isp         An NvMedia ISP %NvMediaISP handle.
 * \param[in] syncObjType Determines how @a nvsciSync is used by @a isp.
 * \param[in] nvsciSync   The %NvSciSyncObj to be registered with @a isp.
 * \param[in] fenceType   A handle to \ref NvMediaISPEOFfenceType if
 *                        @a syncObjType is of type \ref NVMEDIA_EOFSYNCOBJ or
 *                        \ref NVMEDIA_EOF_PRESYNCOBJ. For
 ...
 * Possible values are:
 * - ::NVMEDIA_STATUS_OK if the function is successful.
 * - ::NVMEDIA_STATUS_BAD_PARAMETER if @a isp is NULL or
 ...
 *         if @a nvsciSync is already registered with the same @a isp
...
/**
 * \brief Unregisters an \ref NvSciSyncObj with %NvMediaISP.
 *
 ...
 *
 * \param[in] isp       An NvMedia ISP %NvMediaISP handle.
 * \param[in] nvsciSync An NvSciSyncObj to be unregistered with @a isp.
 *
 * \return ::NvMediaStatus The status of the operation.
 * Possible values are:
 * - ::NVMEDIA_STATUS_OK if the function is successful.
 * - ::NVMEDIA_STATUS_BAD_PARAMETER if @a isp is NULL, or
 *         @a nvsciSync is not registered with @a isp.
 * - ::NVMEDIA_STATUS_ERROR if @a isp was destroyed before this function
 *         was called.
 */
 ...
 * Possible values are:
 * - ::NVMEDIA_STATUS_OK if the function is successful.
 * - ::NVMEDIA_STATUS_BAD_PARAMETER if @a isp is NULL, or if @a nvsciSyncEOF
 *         is not registered with @a isp as either type
 ...
 * \code
 * nvmstatus = NvMediaISPInsertPreNvSciSyncFence(isp, preNvsciSyncFence);
 * nvmstatus = NvMediaISPProcess(isp, params);
 * \endcode
 ...
 * Possible values are:
 * - ::NVMEDIA_STATUS_OK if the function is successful.
 * - ::NVMEDIA_STATUS_BAD_PARAMETER if @a isp is not a valid %NvMediaISP
 *     handle, or @a preNvsciSyncFence is NULL, or if @a preNvsciSyncFence was not
 *     generated with an \ref NvSciSyncObj that was registered with @a isp as
 *     either \ref NVMEDIA_PRESYNCOBJ or \ref NVMEDIA_EOF_PRESYNCOBJ type.
 * - ::NVMEDIA_STATUS_NOT_SUPPORTED if %NvMediaISPInsertPreNvSciSyncFence()
 *     was already called at least %NVM_ISP_MAX_NVSCISYNC_PREFENCES times with
 *     the same @a isp handle before an %NvMediaISPProcess() call.
 */
...
/**
 * \brief Gets EOF \ref NvSciSyncFence for an NvMediaISPProcess() operation.
 *
 ...
 * \code
 * nvmstatus = NvMediaISPProcess(isp, params);
 * nvmstatus = NvMediaISPGetEOFNvSciSyncFence(isp, eofNvsciSyncObj, eofType,
 *                                           eofNvsciSyncFence);
 * \endcode
 ...
 * Possible values are:
 * - ::NVMEDIA_STATUS_OK if the function is successful.
 * - ::NVMEDIA_STATUS_BAD_PARAMETER if @a isp is not a valid %NvMediaISP handle
 *         or @a eofnvsciSyncfence is NULL, or @a eofNvsciSyncObj is not
 *         registered with @a isp as type \ref NVMEDIA_EOFSYNCOBJ or
 *         \ref NVMEDIA_EOF_PRESYNCOBJ.
 * - ::NVMEDIA_STATUS_ERROR if the function was called before
 *         %NvMediaISPProcess() was called.
 */
